...
## Why isn't my Open TutorAI updating?

To update your local Docker installation of Open TutorAI to the latest version available, you can either use **Watchtower** or manually update the container. Follow either of the steps provided below to be guided through updating your existing Open TutorAI image.
...
For more details on environment variable settings, check the [Open TutorAI documentation on security variables](https://docs.openwebui.com/getting-started/env-configuration#security-variables).

## Automatically Updating Open TutorAI with Watchtower

You can use [Watchtower](https://containrrr.dev/watchtower/) to automate the update process for Open TutorAI. Here are three options:
...
You can run Watchtower as a separate container that watches and updates your Open TutorAI container:
...
This will start Watchtower in detached mode, watching your Open TutorAI container for updates every 5 minutes.

### Option 3: Integrating Watchtower with a `docker-compose.yml` File

You can also integrate Watchtower with your `docker-compose.yml` file to automate updates for Open TutorAI (**For Nvidia GPU support, add `--gpus all` to the docker run command**):
...
In this example, Watchtower is integrated with the `docker-compose.yml` file and watches the Open TutorAI container for updates every 5 minutes.
