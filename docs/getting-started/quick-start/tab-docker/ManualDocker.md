...
Follow these steps to install Open TutorAI with Docker.

## Step 1: Pull the Open TutorAI Image

Start by pulling the latest Open TutorAI Docker image from the GitHub Container Registry.
...
- **Port Mapping (`-p 3000:8080`)**: Exposes the TutorAI on port 3000 of your local machine.
...
To connect Open TutorAI to an Ollama server located on another host, add the `OLLAMA_BASE_URL` environment variable:
...
After the container is running, access Open TutorAI at:
