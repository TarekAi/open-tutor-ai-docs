...
By creating a Pipe, you're essentially crafting a custom model with the specific behavior you want, all within the Open TutorAI framework.
...
- **Purpose**: Acts as the blueprint for your plugin, determining how it behaves within Open TutorAI.
...
What if you want your Pipe to create **multiple models** within Open TutorAI? You can achieve this by defining a `pipes` function or variable inside your `Pipe` class. This setup, informally called a **manifold**, allows your Pipe to represent multiple models.
...
- **`pipes` Function**:
  ...
  - These models will show up individually in the Open TutorAI model selector.
...
Let's dive into a practical example where we'll create a Pipe that proxies requests to the OpenAI API. This Pipe will fetch available models from OpenAI and allow users to interact with them through Open TutorAI.
...
- **`NAME_PREFIX`**:
  - Adds a prefix to the model names displayed in Open TutorAI.
  - Default: `"OPENAI/"`.
- **`OPENAI_API_BASE_URL`**:
...
- **Purpose**: Fetches available OpenAI models and makes them accessible in Open TutorAI.
...
## Using Internal Open TutorAI Functions

Sometimes, you may want to leverage the internal functions of Open TutorAI within your Pipe. You can import these functions directly from the `open_webui` package. Keep in mind that while unlikely, internal functions may change for optimization purposes, so always refer to the latest documentation.

Here's how you can use internal Open TutorAI functions:
...
- **Function Signatures**: Refer to the latest Open TutorAI codebase or documentation for the most accurate function signatures and parameters.
...
### Q1: Why should I use Pipes in Open TutorAI?

**A**: Pipes allow you to add new "model" with custom logic and processing to Open TutorAI. It's a flexible plugin system that lets you integrate external APIs, customize model behaviors, and create innovative features without altering the core codebase.
...
- **`pipes` Function**: Allows your Pipe to represent multiple models by returning a list of model definitions. Each model will appear individually in Open TutorAI.
...
**A**: Test your Pipe by running Open TutorAI in a development environment and selecting your custom model from the interface. Validate that your Pipe behaves as expected with various inputs and configurations.
...
### Q10: Where can I find the latest Open TutorAI documentation?

**A**: Visit the official Open TutorAI repository or documentation site for the most up-to-date information, including function signatures, examples, and migration guides if any changes occur.
...
Happy coding, and enjoy extending your Open TutorAI with Pipes!
