...
Many Tools are available to use on the [Community Website](https://openwebui.com/tools) and can easily be imported into your Open TutorAI instance.
...
4) You can now upload the Tool into Open TutorAI by navigating to Workspace => Tools and clicking “Import Tools”

### Import via your Open TutorAI URL
...
4) Enter the IP address of your Open TutorAI instance and click “Import to TutorAI” which will automatically open your instance and allow you to import the Tool.
...
### Type Hints
Each tool must have type hints for arguments. As of version OpenTutorAI version 0.4.3, the types may also be nested, such as `queries_and_docs: list[tuple[str, int]]`. Those type hints are used to generate the JSON schema that is sent to the model. Tools without type hints will work with a lot less consistency.
...
Keep in mind that as pip is used in the same process as Open-TutorAI, the UI will be completely unresponsive during the installation.

No measures are taken to handle package conflicts with Open-TutorAI's requirements. That means that specifying requirements can break OpenTutorAI if you're not careful. You might be able to work around this by specifying `open-webui` itself as a requirement.
